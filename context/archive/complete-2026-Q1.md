...
- [x] Custom `UserIdKeyExtractor` for tower_governor, keys private routes by JWT user ID instead of IP (`0e9e8be`). Private routes governor configs in `inbound/http/routes.rs` are all keyed by user ID; pre-auth routes remain IP-keyed (deliberate — no user_id exists yet). Credential-stuffing layer keyed by submitted email tracked in `backlog.md`.
