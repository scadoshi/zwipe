...
- [ ] **Public stats strip on zwipe.net** — backend `GET /api/marketing/stats` already returns `{cards_swiped, searches, decks_created}` aggregated across all users (shipped on `feat/user-metrics`); CF Cache Rule fronts it at the edge with 1h TTL. What's deferred is the zite component that renders the numbers in the home-page hero. Plan in [context/plans/zite-public-metrics.md](../plans/zite-public-metrics.md). Cut a fresh branch from `main` after `feat/user-metrics` merges.
