...
pub static GUIDES: &[Guide] = &[
    ...
    Guide {
        ...
        blocks: &[
            ...
            Block::P("Every add saves instantly and syncs across your devices. Skips stick too: swipe left and that card stays out of this deck, even after you close the app. Each deck also keeps your place, so you can hop between decks and pick up where you left off."),
    ...
    Guide {
        ...
        blocks: &[
            ...
            Block::P("A left swipe keeps that card out of this deck for good, even after you reopen the app. Clear skips in the deck's More menu brings them back. The swipe-memory guide covers this in full."),
            Block::H2("Synergy"),
            ...
            Block::H2("Undo"),
            Block::P("Swipe down to undo your last swipe; undoing an add removes the card again, and undoing a skip brings the card back. Your place and undo history travel with each deck, so leaving and returning picks up where you left off. Refresh (or changing the filter) starts a fresh stack and clears the history."),
            Block::Note("At the very end of the stack, swipe down on the empty card to step back through your last swipes."),
        ],
    },
    Guide {
        slug: "remove-cards",
        ...
        blocks: &[
            ...
            Block::Note("A card you remove also stops showing up when adding to this deck, the same as a skip. Clear the deck's skips to bring removed cards back into the pool."),
        ],
    },
    Guide {
        slug: "swipe-memory",
        ...
        category: "Build",
        blocks: &[
            Block::Lead("Zwipe remembers the cards you pass on. Skip a card and it stays out of that deck's suggestions, so you never have to swipe past the same card twice."),
            Block::H2("Skips stay skipped"),
            Block::P("A left swipe on the add screen tells Zwipe you don't want that card in this deck. It stops appearing while you build, and it stays gone after you close and reopen the app. Removing a card from a deck does the same, so trimmed cards don't come back as suggestions."),
            Block::H2("It's per deck"),
            Block::P("Memory is scoped to the deck you're building. Skipping a card in one deck never hides it in another, so each deck's suggestions reflect only your choices for that deck."),
            Block::H2("Pick up where you left off"),
            Block::P("Every deck keeps its place in the stack, along with your recent swipes for undo. Jump out to tweak another deck and come back, and Zwipe drops you right back where you were instead of starting over."),
            Block::H2("Changed your mind? Clear skips"),
            Block::P("Open the deck's More menu and tap Clear skips. Every card you skipped or removed for that deck comes back into the pool, and swiping starts fresh."),
    ...
    Guide {
        ...
        blocks: &[
            ...
            Block::H2("Sorting"),
            Block::P("Sort by `Name`, `Mana value`, `Power`, `Toughness`, `Rarity`, `Release Date`, `Price`, `Popularity`, or `Random`. Sorting only reorders the set you're looking at; it never changes which cards are eligible. Your filter and sort follow you between screens."),
            Block::H2("Defaults and Reset"),
            Block::P("Every screen starts from a default view, and that default isn't always empty. `Reset` returns you to it:"),
            Block::Bullets(&[
            ...
            Block::P("The dot on the `Filter` button means you've changed something from that default, a filter or a different sort. `Reset` clears the change and drops you back to the default. Synergy is a separate mode, so it stays however you set it."),
            Block::Note("These built-in defaults never light the dot, since you didn't set them: your commander's colors, format legality, the popularity or synergy order, and lands leaving once you're at target."),
            Block::H2("Categories"),
