...
- [ ] **Migrate prod off the home server to a VPS** (decided 2026-06-10 — home server is going away soon, so this is a must, not a maybe; do it before Android grows the user base). Full plan with options, phased cutover (tunnel CNAME flip), rollback, and decommission checklist: `context/plans/vps-migration.md`. Default shape: Hetzner CPX31 (x86, 8GB, US), Tailscale-only SSH, copy JWT_SECRET, restore from the existing R2 dumps (doubles as the never-run disaster-recovery drill). — submitted 2026-06-10; carries Archidekt import and the min-version gate. Once approved and propagated, every install ≥1.0.5 is force-updatable.
