...
- [ ] **Migrate prod off the home server to a VPS** (decided 2026-06-10 — home server is going away soon, so this is a must, not a maybe; do it before Android grows the user base). Small instance (Hetzner ~€4–7 tier or DO/Vultr equivalent, US region) running exactly what runs today: zerver, zervice, Postgres, systemd units. Steps: provision + harden (ssh keys only, firewall), Postgres dump/restore, copy `.env` (rotate JWT_SECRET? decide — rotating logs everyone out), repoint CI/CD deploy target, flip DNS at Cloudflare (proxy stays in front), smoke test `/health` + login + import. Then set up nightly `pg_dump` shipped offsite as a recovery story. Rationale: uptime accountability for a live app, blast-radius isolation (no forwarded ports into a home LAN), residential ISP ToS. — submitted 2026-06-10; carries Archidekt import and the min-version gate. Once approved and propagated, every install ≥1.0.5 is force-updatable.
