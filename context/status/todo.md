...
- [ ] **"Update required" screen flashes when applying filters on add/remove cards** (observed 2026-06-11) — hitting Apply filter on the add-cards or remove-cards screen briefly swaps in the blocking "Update required" screen before the stack renders. Likely the min-version gate's root-component swap reacting to a transient/unset state during the rerender (`session_upkeep.rs` poll + root gate check) rather than a real version mismatch — the screen should only appear on a confirmed below-minimum answer, never while state is unknown. Repro: open add or remove cards → apply a filter → watch for the flash.
