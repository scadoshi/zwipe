...
async fn metadata_endpoints_return_distinct_values(pool: sqlx::PgPool) {
    seed_cards(
        ...
        &[
            ...
            card("Serra Angel")
                ...
                .artist("Bob Art")
                .set("M10", "Magic 2010"),
            card("Ancestral Recall").type_line("Instant").lang("ja").artist("Alice Art"),
        ],
    )
    .await;

    let artists = list(&app_of(pool.clone()), "/api/card/artists").await;
    assert_eq!(artists, vec!["Alice Art", "Bob Art"], "distinct + sorted, no duplicate Alice");

    let sets = list(&app_of(pool.clone()), "/api/card/sets").await;
    assert!(sets.contains(&"Magic 2010".to_string()) && sets.contains(&"Test Set".to_string()), "{sets:?}");
    ...
    assert_eq!(keywords, vec!["flying", "haste", "vigilance"], "{keywords:?}");
    ...
    assert!(!types.iter().any(|t| t == "of" || t == "the"), "stop words excluded: {types:?}");
}
...
async fn printings_returns_all_printings_of_an_oracle(pool: sqlx::PgPool) {
    ...
    seed_cards(
        &pool,
        &[
            card("Reprinted Card").oracle(Some(shared)).set("A", "Alpha Set"),
            card("Reprinted Card").oracle(Some(shared)).set("B", "Beta Set"),
            card("Unrelated Card"), // its own oracle, must not appear
        ],
    )
    .await;

    let (status, body) = app.get(&format!("/api/card/{shared}/printings"), None).await;
    ...
    assert!(
        printings.iter().all(|c| c["scryfall_data"]["name"] == "Reprinted Card"),
