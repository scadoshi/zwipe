...
- **`impl AsRef<str>` on Validators**: Domain validator constructors should accept `impl AsRef<str>` rather than `&str`. This lets callers pass both `&str` literals and owned `String` values without borrowing. Inside the function, bind with `let raw = raw.as_ref();` to get a `&str` for further use. Cascade the change to all request constructors that receive `&str` and pass it into the validator — update their signatures too. In `Deserialize` impls, use `Username::new(raw)` (passes owned `String`) rather than `Username::new(raw.as_str())`. At SQLx `TryFrom` call sites, pass `value.field` (owned) rather than `&value.field` (borrow). At frontend signal call sites, pass `signal()` (owned `String`) rather than `&signal()`.
- **`impl Into<String>` for name-like types**: `DeckName::new` already used `impl Into<String>` which allows both `&str` and `String`. Note: `&String` coerces to `&str` which coerces to `String` via `Into`, so `DeckName::new(&value.name)` compiled even though `&String: Into<String>` is not directly implemented — safer to just pass `value.name` (owned).
...
- **GroupCards Extension Trait**: Same pattern as FilterCards — `impl GroupCards for Vec<Card>` with `group_by(GroupByOption) → Vec<CardGroup>`. Bucket-based O(n) classification with fixed-order labels; empty groups filtered out. Three modes: CardType (priority match on type_line), Cmc (floor + cap at 6), Color (WUBRG + multicolor + colorless).
- **Filter → Group In-Memory Pipeline**: Chain `filter_by` then `group_by` on the same `Vec<Card>` for instant client-side data transformation. Server hit once on mount; all subsequent filter/group changes are local.
- **Unified `.screen` Fixed-Frame Layout**: All 14 screens wrap content in `div.screen` (`position: fixed; inset: 0; display: flex; flex-direction: column`). `.page-header` and `.util-bar` are `flex-shrink: 0` items. `.screen-content` fills remaining space with `flex: 1; overflow-y: auto; min-height: 0`. No body scroll, no sticky positioning, no per-screen inline layout styles. Safe-area insets on `.screen` via `env(safe-area-inset-top/bottom)`. This pattern makes Dioxus DOM patching across route changes safe — identical structural CSS prevents scroll/position state leakage.
- **Commander Deduplication in Card List**: Fetch commander separately via get_deck_profile → get_card, check `cards.iter().any(|c| c.scryfall_data.id == commander_id)` before inserting to avoid duplicates.
- **Data-Driven Warning Dialogs**: Instead of warning based on abstract state comparison (e.g. "is the new limit lower?"), check actual data against the limit (e.g. `max_entry_quantity() > *new_max`). Eliminates false-positive warnings when no data actually overflows. Pattern: fetch real data on mount, store derived metric in a signal, compute `would_truncate` memo reactively.
- **Basic Land Exemption Pattern**: Basic lands are exempt from copy-max limits in all formats (singleton, standard). Detection via `type_line.contains("basic land")` matches backend pattern. Frontend: `change_quantity` takes `is_basic_land` flag to skip copy-max checks; singleton non-basic cards show "remove" button instead of -/+ controls; EditDeck `max_entry_quantity` filters out basic lands for truncation warnings.
