...
- [ ] **Track 1.0.9 (build 42) App Store review + propagation** — submitted for review 2026-06-23. Supersedes 1.0.8 (build 38); builds 39–41 were intermediate (app-icon iteration), **build 42 (1.6× icon) is the keeper**. A UI-consistency pass + a brand-new app icon: **new app icon** — the project's ASCII "Z" mark (Gruvbox cream `#ebdba2` on `#282828`), generated via the **asciier** tool (`github.com/scadoshi/asciier.git`) at JetBrains Mono / line-height 0.9× / size 1.6×; recipe in `context/ops/ios/appstore-icon-update.md`. **Self-hosted JetBrains Mono** (full font bundled via `asset!()`, drops the CDN `@import` — fixes the Android-WebView home-screen logo block glyphs; no-op on iOS, removes a startup network dependency). **Profile rework** — username/email/password edits are now bottom sheets, Delete account lives behind a `More` sheet, Account/Preferences cards with in-card section headers. **Deck view** — section subtitles (Profile/Stats/Warnings + the five charts) moved **inside** their carded elements with dividers (shared `.card-header`/`.card-title`). **Deck list** — each entry is now one flowing row: name + accent-colored stat chips (count / format / command-zone), and the **count chip turns warning-yellow when the deck is an illegal size for its format**; long names wrap. **Home flavor card** cached app-wide with a 1h TTL (stale-while-revalidate) so rapid navigation no longer re-hits the rate-limited search or blanks the quote. **Deck-size rules fixed** in `zwipe-core` — Oathbreaker/Brawl/Historic Brawl/Gladiator now use correct legal sizes (feeds both the deck-list chip and the server's `validate_deck` warnings). Smaller: "To deck" → "To mainboard"; Printings sheet accent; opaque chart skeleton; Gruvbox text yellow-leaned (`#ebdbb2`→`#ebdba2`). Workspace version bumped 1.0.6→1.0.9 (all crates) so `CARGO_PKG_VERSION` matches the store version; once approved + propagated, 1.0.9 is the floor for `MIN_CLIENT_VERSION` gating.
  - **Server side DEPLOYED to prod 2026-06-23** — the `zwipe-core` format-size fix + version bump pushed → `zerver` redeployed (prod root reports `version: 1.0.9`, `/health` green; corrected deck-size warnings live) and `zite` redeployed to zwipe.net (Gruvbox text tweak). iOS build 42 = version 1.0.9, Apple Distribution, Xcode 26.5, uploaded via Transporter (icon-only repack over the build-40 `.app`; build 39 used the full `appstore-update.md` flow with the 26.4→26.5 cache wipe). Android emulation (Pixel_9a) confirmed running this code earlier — JDK build gotcha in `context/ops/android.md`.
- [ ] **Track 1.0.8 (build 38) App Store review + propagation** — submitted for review 2026-06-14. Supersedes 1.0.6 (build 35, synergy-ordered suggestions + remove-screen stage/promote toggle + board-sync fix) and the 1.0.7 UI-revamp build(s): the mobile look overhaul (background grid + darker `--bg-sink` content, opaque floating panels/cards/inputs/chips with shadows, real mana pips, bottom-sheet preferences with per-theme swatches, shared dialog rules; What's New in `context/ops/ios/app-store-submission/form-fields.md`). **1.0.8 adds the skeleton-polish pass** — bordered input-box form skeletons, two-line deck-list rows, and `.skeleton-deck-list-item` aligned to `border-secondary` (commit `d35b2d26`). Cumulative feature floor still carries synergy-ordered suggestions, Archidekt import, the min-version gate, the 401/health fixes, and first-run hints. Once approved + propagated, 1.0.8 is the floor for `MIN_CLIENT_VERSION` gating. Watch ASC review states.
  - Build/sign/package process documented in `context/ops/ios/appstore-update.md`; signed `Zwipe.ipa` built 2026-06-14 (version 1.0.8, build 38, Apple Distribution, Xcode 26.5 SDK keys) and uploaded via Transporter.
