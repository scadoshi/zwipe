...
impl TryFrom<ImageUris> for JsonValue {
    type Error = serde_json::Error;
    fn try_from(value: ImageUris) -> Result<Self, Self::Error> {
        serde_json::to_value(value)
    }
}

impl Decode<'_, Postgres> for ImageUris {
    fn decode(
        value: <Postgres as sqlx::Database>::ValueRef<'_>,
    ) -> Result<Self, sqlx::error::BoxDynError> {
        let json_value = <JsonValue as Decode<Postgres>>::decode(value)?;
        let related_card: ImageUris = serde_json::from_value(json_value)?;
        Ok(related_card)
    }
}

impl Type<Postgres> for ImageUris {
...
impl Encode<'_, Postgres> for ImageUris {
    fn encode(
        self,
