...
- [ ] **HTTP response compression** — add `tower-http` `CompressionLayer` to Axum router; `/api/card/search` body drops from ~39KB to ~5-8KB → ~30-60ms saved on public timing. See item #2 in `context/ops/latency-optimization.md`.
- [ ] **Lower default search page size** — `CardFilter::default_limit()` is 100; lower to 25 and paginate via existing `offset`. Pairs with compression for faster perceived first-paint on filter screens.
