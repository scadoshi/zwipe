...
- [ ] **DFC handling — render front face for transform/MDFC + add flipper for all double-faced layouts** — Scryfall's data shape splits by layout:
  - **Already render correctly today**: `layout = meld` (e.g. `Urza, Lord Protector`) — each meld piece is a separate Scryfall row with its own top-level `image_uris`. `layout = split` (`Wear // Tear`) and `layout = adventure` (`Lovestruck Beast`) also work — single image with both halves baked in.
  - **Do NOT render today**: `layout = transform` (`Delver of Secrets`), `layout = modal_dfc` (`Valki, God of Lies`). For these, top-level `image_uris` is NULL; image URLs live in `card_faces[0].image_uris.large` and `card_faces[1].image_uris.large`. Zwiper has zero `card_faces` references anywhere — every render site (`card_row.rs:49`, `printing_sheet.rs:125-126`, `add.rs:158/561/955`) reads only top-level `image_uris`, so these cards render as blank image surfaces. The card visibility fix now surfaces these in search and commander pickers — users will hit blank Delvers immediately.
  - **Two-step task**: (1) add `Card::primary_image_url()` in zwipe-core that falls back to `card_faces[0].image_uris.large` when top-level is `None`, and replace the 5 ad-hoc call sites with it — unblocks transform/MDFC rendering; (2) add a flipper affordance (tap/click to swap between `card_faces[0]` and `card_faces[1]`) on every image surface — likely a `<FlippableCardImage>` shared component. Flipper applies to all double-faced layouts including meld (where the "back" is a separately-rowed melded result, so the flipper might cross-reference via `all_parts` or similar — needs more design). Single-faced layouts (normal, split, adventure) don't need the flipper.
