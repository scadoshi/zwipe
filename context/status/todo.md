...
- [ ] **Migrate prod off the home server to a VPS** (planned 2026-06-10, DEFERRED — at ~20 users the home server is fine; revisit when users grow meaningfully, premium revenue starts, or the home server's retirement becomes real). Ready-to-run plan with options, phased tunnel-CNAME cutover, rollback, and decommission checklist: `context/plans/vps-migration.md`. — submitted 2026-06-10; carries Archidekt import and the min-version gate. Once approved and propagated, every install ≥1.0.5 is force-updatable.
...
- [ ] **Missing-auth responses return 500, should be 401** (noted 2026-06-11) — every private route answers a request with no Authorization header with a 500 (auth middleware error mapping), which pollutes error logs and misleads probes. Pre-existing, cosmetic, all routes equally. Map missing/invalid header to 401.
- [ ] **`/health` path drifted** (noted 2026-06-11) — `ops/infra/backups.md` and habit say `curl .../health`, but that path 404s (dev and prod alike); the live check is `GET /` (returns status ready + version). Fix the docs or restore the route.
- [ ] **Verification screen: resend throttle + refresh** (noted 2026-06-11) — on the email-verification screen, throttle the resend button (cooldown, server-side too — don't let anyone mass-send email through us) and add a refresh affordance so users can check "did it land?" without leaving the screen.
