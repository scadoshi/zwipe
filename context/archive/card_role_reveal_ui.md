...
**Status: DONE 2026-07-12.** Part 1 (block-quote the revealed otags) SHIPPED (`bedcf76a`).
Part 2 (per-chip clickable hint text) was **superseded** — the otag-education need was met
instead by in-app education pages + a help affordance (`9bfe784c`, and the `help` prop on
`CardRoleChips`), with otags shown as raw slugs (`73242c98`). No live remaining work.
