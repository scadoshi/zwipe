...
**Status: PARKED (sketched 2026-07-29), but the reason for parking it was disproven on 2026-09-21. Still not scheduled; the case is now stronger than when it was written.**
...
Integration tests that exercise the FULL client path: `ZwipeClient` (reqwest) → real HTTP → axum router → sqlx → Postgres, instead of testing each side of the wire separately. Catches the thin-translation-layer bugs nothing else can: status→`ClientError` mapping against real responses, header/auth attachment, real-payload decoding, refresh flows.

## The key insight: it lives on the ZWIPER side

zwiper already links zerver; zerver must never know its consumer. So the harness is zwiper integration tests (`zwiper/tests/`) that construct zerver's router in-process, bind it to an ephemeral port, and point a real `ZwipeClient` at `127.0.0.1:<port>`. No architectural change, no code moves, dependency arrow untouched.
