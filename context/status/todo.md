...
- [ ] **Track 1.0.6 (build 33) App Store review + propagation** — build 32 submitted 2026-06-11; build 33 uploaded same day and supersedes it (adds the update-required-flash fix below — swap the selected build on the 1.0.6 version page in ASC). Headline feature is synergy-ordered card suggestions (deck-aware add screen), plus Archidekt import, the min-version gate, and the 401/health fixes. Supersedes 1.0.5 (build 31). Once approved + propagated it's the floor for `MIN_CLIENT_VERSION` gating. Watch ASC review states; What's New text in `context/ops/ios/app-store-submission/form-fields.md`.
...
- [x] **First-run hints — BUILT 2026-06-11** (server `06f0b441`, client `298c4848`; evolved from "tutorial toasts" to one-time dialogs during design). Shipped: **`hints_shown` jsonb on users** (additive migration; rides every user-returning query: login, get-user, change-username/email) + `PUT /api/user/hint` marking a key shown (idempotent, returns updated user; key constants + validation in `zwipe-core/.../hints.rs`). Client: `use_one_time_hint(key)` hook + `HintDialog` shell (`components/hint_dialog.rs`) — auto-opens on first visit per account, reports shown (fire-and-forget, optimistic), persists session so it survives restarts. Three hints live: **add-cards swipe vocabulary** (right add / left skip / up maybeboard / down undo + synergy-ordering line), **remove-cards** (right remove / left keep / up to maybeboard / down undo + board-chips line), **first-deck welcome** on deck profile (points at Cards and More-actions import). Add/remove keep a grayed "?" util-bar button that reopens the dialog forever. New hints = new keys, no migration. Related (built same day): below-minimum deck warning row carries Add cards / Import remedy buttons. Live-tested: register → `{}`, marks accumulate + idempotent, 422 bad key, 401 no auth. NOTE: hint persistence needs the server deploy; until then dialogs re-open per screen visit and mark calls 404 harmlessly.
