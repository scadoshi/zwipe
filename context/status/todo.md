...
- [ ] **Track 1.0.9 (build 39) App Store review + propagation** — submitted for review 2026-06-22. Supersedes 1.0.8 (build 38). A UI-consistency + Android-readiness pass: **self-hosted JetBrains Mono** (full font bundled via `asset!()` in `zwipe.rs`, dropping the flaky CDN `@import` — the fontsource subsets omit U+2580–U+259F, so the home-screen ASCII logo's block glyphs fell back to a misaligned symbol font on Android WebView; the full font carries them at the monospace advance width. No-op on iOS, and removes a startup network dependency); **profile rework** — username/email/password edits are now bottom sheets, Delete account lives behind a `More` sheet, and the screen splits into **Account** and **Preferences** cards with in-card section headers (Preferences shows current theme + dark mode read-only; Change opens the existing `PreferencesSheet`); **deck-view** section subtitles (Profile/Stats/Warnings + the five charts) moved **inside** their carded elements with dividers, matching the profile cards (shared `.card-header`/`.card-title`); **deck list** gained a name/stats divider and long-name wrapping; card mover **"To deck" → "To mainboard"**; Printings sheet title recolored to the shared bottom-sheet accent. Workspace version bumped 1.0.6→1.0.9 across all crates so `CARGO_PKG_VERSION` (the value the min-version gate reports) matches the store version. Once approved + propagated, 1.0.9 is the floor for `MIN_CLIENT_VERSION` gating.
  - Built/signed/packaged 2026-06-22 per `context/ops/ios/appstore-update.md` (Xcode 26.5; iOS target cache wiped for the 26.4→26.5 SDK re-link); `Zwipe.ipa` = version 1.0.9, build 39, Apple Distribution, uploaded via Transporter. Android emulation (Pixel_9a) confirmed running this same code this session — JDK build gotcha captured in `context/ops/android.md`.
