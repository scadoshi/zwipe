...
- **Conditional CSS Grid Columns**: Use a CSS modifier class (e.g. `.no-qty`) that overrides `grid-template-columns` to remove a column. Conditionally omit the corresponding `<span>` from the DOM. CSS grid assigns children in source order, so simply omitting the element avoids misalignment — no `grid-column` hacks needed.
