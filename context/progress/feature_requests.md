...
| # | Feature | Impact | Effort | Priority | Notes |
|---|---------|--------|--------|----------|-------|
| 11 | Persist skipped cards per deck across sessions | High | M | ✅ Shipped (1.3.0) | Skips **and deliberate removals** persist server-side (`deck_card_suppressions`); the deck-aware search stops serving them, with a "Clear skips" button in the deck view's More sheet. Design + as-built notes: [`../archive/swipe_memory.md`](../archive/swipe_memory.md). Requested a second time via App Store review 2026-07-02 (see sources above). |
