...
- [ ] **Migrate prod off the home server to a VPS** — **Phase 0 EXECUTED 2026-06-13.** Hetzner CPX31 (4 vCPU / 8GB / 160GB, Hillsboro OR, Ubuntu 26.04, x86), tailnet IP `100.114.251.8`, Hetzner name `zerver-prod`, system hostname `zerver-prod`. Hardened: `scadoshi` user (key-only, NOPASSWD sudo — scope down before cutover), ufw default-deny inbound with only `tailscale0` allowed (zero public ports; SSH is tailnet-only), public-IP SSH confirmed dead. Stack: Postgres 18 + least-privilege `zwipe` role, Rust 1.96, sqlx-cli, gcc linker symlink, `/var/log/zwipe`. `zerver` built from `main` (`SQLX_OFFLINE=true`, cold release build ~2m15s), running as the `zerver.service` systemd unit (enabled on boot, auto-restart). All 22 migrations applied; `zervice` seeded 115,803 cards. Smoke-tested green: `/`, `/health` (server+DB), register, login, `/api/user/preferences`, `/api/user/metrics`. **Fresh staging box — no prod data, fresh JWT secret, placeholder Resend key.** NOT cut over (Phase 1 still pending: needs Cloudflare tunnel → `api.zwipe.net` route flip + final pg_dump/restore + zynegry worker + GitHub runner). Pending before cutover: (a) the card-search 500 below, (b) Resend key, (c) the kernel reboot is still queued on the box (apt pulled 7.0.0-22; reboot deferred so it didn't kill the build — reboot to load it + verify systemd auto-start). Plan: `context/plans/vps-migration.md`.
- [ ] **`POST /api/card/search` returns 500 (panic) — found 2026-06-13 on the VPS Phase-0 box, almost certainly also on prod (identical `main` + `Cargo.lock`).** With a valid token the route 500s; the request never reaches the handler (`log_500` never fires) — it's a panic caught by `CatchPanicLayer`, and the panic message lands nowhere observable (not in the `/var/log/zwipe` tracing log, not in journald), so there's a real **observability gap**: panics on caught routes are invisible. The `UserIdKeyExtractor`/governor is fine (other governed routes — `/api/user/preferences`, `/api/user/metrics` — return 200; missing-key correctly 401s). This is the commander-autocomplete endpoint, so if it's live on prod, commander search is silently 500ing for users (the in-app symptom would look like "no results"). Repro: `POST /api/card/search` with `{}` or `{"name":"Sol Ring"}` + Bearer token → 500. Next: (1) **first verify whether prod reproduces** (decides whether it's a cutover blocker or a pre-existing prod bug); (2) make `CatchPanicLayer` log the panic (custom handler → `tracing::error!`) so it's diagnosable; (3) find the panic in the card search path (`zerver/.../outbound/sqlx/card/mod.rs` query builder / `Card` TryFrom mapping — `indexing_slicing`/`unwrap` are clippy-warn not deny).
