...
- **Per-Screen Scroll Containers**: When `html { overflow: hidden }` and `body { overflow: clip }`, each screen must create its own scroll container with `height: 100vh; overflow-y: auto`. Page header is `position: sticky` at ~5rem height, requiring matching `padding-top` on content.
