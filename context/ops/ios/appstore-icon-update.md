...
For a fresh release build (icon + Rust changes), follow [appstore-update.md](appstore-update.md) and let step 3 swap the icons as part of the normal flow.
...
This assumes you ran `dx build --release --platform ios` recently and `~/Developer/zwipe/target/dx/zwipe/release/ios/Zwipe.app` already exists. If not, do that first (step 1 of `appstore-update.md`).

```bash
APP=~/Developer/zwipe/target/dx/zwipe/release/ios/Zwipe.app

# Stage the icon catalog (same Contents.json as appstore-update.md step 3).
...
Same as [appstore-update.md](appstore-update.md) step 6 — drag `~/Developer/zwipe/Zwipe.ipa` into Transporter and click Deliver.
