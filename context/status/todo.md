...
- [ ] **Track 1.0.6 (build 32) App Store review + propagation** — submitted 2026-06-11; headline feature is synergy-ordered card suggestions (deck-aware add screen), plus Archidekt import, the min-version gate, and the 401/health fixes. Supersedes 1.0.5 (build 31). Once approved + propagated it's the floor for `MIN_CLIENT_VERSION` gating. Watch ASC review states; What's New text in `context/ops/ios/app-store-submission/form-fields.md`.
...
- [ ] **Resend-verification throttle + refresh** (noted 2026-06-11; PLANNED — full design in `context/plans/resend-verification-throttle.md`) — server throttle is NOT meaningfully in place today (`/api/auth/resend-verification` only under the broad `private_config`, burst 500). Plan: dedicated per-route limiter (burst 1, then 1/60s per user, with the `unauthorized_on_missing_key` handler) — server is patchable now, no app release. Client (on the profile email row, not a separate screen): Resend button greys out immediately with a 60s countdown matching the server window, plus a "Check again" button that re-fetches the user so the badge flips to Verified without leaving the screen. Client half rides with the next build (32 already shipped without it).
...
- [ ] **"Update required" screen flashes when applying filters on add/remove cards** (observed 2026-06-11) — hitting Apply filter on the add-cards or remove-cards screen briefly swaps in the blocking "Update required" screen before the stack renders. Likely the min-version gate's root-component swap reacting to a transient/unset state during the rerender (`session_upkeep.rs` poll + root gate check) rather than a real version mismatch — the screen should only appear on a confirmed below-minimum answer, never while state is unknown. Repro: open add or remove cards → apply a filter → watch for the flash. Diagnostic clue: does NOT happen on the deck-cards screen's Apply filter — that screen filters the already-loaded deck locally, while add/remove rebuild the swipe stack, so the trigger is likely in the stack rebuild/remount path rather than filtering itself.
