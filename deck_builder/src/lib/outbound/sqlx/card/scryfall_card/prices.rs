...
use crate::domain::card::models::scryfall_card::prices::Prices;

impl TryFrom<Prices> for JsonValue {
    type Error = serde_json::Error;
    fn try_from(value: Prices) -> Result<Self, Self::Error> {
        serde_json::to_value(value)
    }
}

impl Decode<'_, Postgres> for Prices {
    fn decode(
        value: <Postgres as sqlx::Database>::ValueRef<'_>,
    ) -> Result<Self, sqlx::error::BoxDynError> {
        let json_value = <JsonValue as Decode<Postgres>>::decode(value)?;
        let related_card: Prices = serde_json::from_value(json_value)?;
        Ok(related_card)
    }
}

impl Type<Postgres> for Prices {
...
impl Encode<'_, Postgres> for Prices {
    fn encode(
        self,
