...
Shared Dioxus UI components and CSS for Zwipe, consumed by the app (`zwiper`), the website (`zite`), and the owner's portfolio site (via a GitHub git dependency). Rendering and styling live here once so the surfaces never drift.
...
External consumers include these strings directly; the app and site inline them through their own asset pipelines.
